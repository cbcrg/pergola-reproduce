...
plot_name <- "sushi_jaaba_scores_annot_highlight"

{
    if (image_format == 'tiff' | image_format == 'tif') {
        tiff(paste(plot_name, variable_name, ".", image_format, sep=""), res=300)
    }
...
for (bedg_i in seq_along(data_bedgraph_variable)) {
  ...
  plotBedgraph(data_bedgraph_variable_comp[[bedg_i]], chrom, chromstart, chromend,
               transparency=.10, color=SushiColors(2)(2)[2])
  plotBedgraph(data_bedgraph_variable[[bedg_i]], chrom, chromstart, chromend, transparency=.80,
